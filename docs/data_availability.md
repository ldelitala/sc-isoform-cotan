...
repository (multi-GB); they live on athena under `data/project_files/<dataset>/`.
...
| Samplesheet | `athena:runs/arrigoni/samplesheet.csv` (sample `banchmark_mix`) |
| Analysis workspace | `athena:data/project_files/arrigoni/` |
...
| Samplesheet | `athena:runs/ding/cortex_2/samplesheet.csv` (sample `cortex_2`) |
| Analysis workspace | `athena:data/project_files/ding/cortex_2/` |
...
inside a run directory (`runs/` is gitignored and exists only there):

```bash
cd /data/lorenzo_delitala/runs/<dataset>
