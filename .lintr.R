...
# and `library()` in `src/analysis/` is reported as "no visible global function
# definition". Run it locally in the analysis environment when you want that check:
#   lintr::lint_dir("src/analysis", linters = lintr::linters_with_defaults(
...
# `src/analysis/deprecated/` holds superseded code kept for provenance only (S7). It is
...
exclusions <- list(
  normalizePath("src/analysis/deprecated/06_det_dtu.R", mustWork = FALSE),
  normalizePath("src/analysis/deprecated/02_ext_dtu.R", mustWork = FALSE)
)
