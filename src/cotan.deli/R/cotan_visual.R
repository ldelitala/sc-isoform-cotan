...
save_cotan_plots <- function(
  ...
  GDIIn = NULL
    ...
    # Note: For UMAP and Heatmaps, you usually need to define the markers/features
    # you are clustering on first (like the `layersGenes` in the tutorial).
    # If you have those established, you generate them the exact same way:
    #
    # umap_plot <- UMAPPlot(...) # nolint
    # ggsave(filename = file.path(out_dir, "UMAP_plot.pdf"), plot = umap_plot) # nolint
