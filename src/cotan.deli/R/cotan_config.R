...
#' @export
config_cotan_workflow <- function(
    output_dir = "./",
    ...
    log_header("Configuring COTAN Workflow")
    ...
    log_stat(sprintf("Log file initialized at: %s", log_path))
    log_stat(sprintf("Logging level set to: %d", logging_level))

    log_header("Configuration Complete", is_complete = TRUE)
